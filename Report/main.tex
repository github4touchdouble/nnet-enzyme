...
% TODO: Why didn't we normalize the mass of our sequences <29.01.24, Malte Weyrich> %
...
The non-enzyme dataset, however, contains many sequences with a length of up to $18000$ amino acids.
...
% Kida done
% TODO: Look at comments, not finished <29.01.24, Malte Weyrich> %
...
The architecture is as follows: The input dimension is $2560$.
The hidden layer node counts are $2048$,
...
% We implemented this architecture using TensorFlow, an open-source library for machine learning (see \cite{tensorflow2015-whitepaper}). % TODO: Maybe introduce these details earlier?
...
Therefore, the layer sizes were adjusted. The model's architecture consists of an input layer of $1024$ 
and three hidden layers of $1024$, $512$, and $7$ units. The model also uses batch normalization between the layers, 
activation function LeakyReLU, class weights, and Adam as an optimizer. This model, however, performs worse than the 
FNN using ESM-2 embeddings.
...
The first layer takes ESM-2 embeddings of size $2560$ as the input and transforms them to $1760$ units, then $1000$, $500$, and finally, $1$ unit. For ProtT5 with a tensor having $1024$ inputs, the layers after the input layer were set to $512$, $256$, and $1$.
...
\begin{figure}[!hb]
\includegraphics[width=0.9\textwidth]{assets/l1_dist_enzymes.png}
\caption{Main class distribution of Split30 (logarithmic scale).}\label{fig:l1_dist_train}
\end{figure}

\begin{figure}[!ht]
\includegraphics[width=0.9\columnwidth]{./assets/l2_dist_all_train_ver.png}
